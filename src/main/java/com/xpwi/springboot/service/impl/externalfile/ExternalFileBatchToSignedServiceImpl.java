...
import com.xpwi.springboot.service.externalfile.ExternalFileBatchToSignedService;
...
/**
 * @program: idea_server
 * @description: 外來文件批量上傳
 * @author: PengFei_Ge
 * @create: 2020-12-29 11:44
 **/
@Service
public class ExternalFileBatchToSignedServiceImpl implements ExternalFileBatchToSignedService {
    @Autowired
    ...
    @Autowired
    private FlowExternalMessageDao flowExternalMessageDao;
                ...
                if(add_list.size()!=0){  // 此处 无效作用  只做逻辑判断
                    ...
                    if(next_signed== null) {  // "簽核完成" ,為查找到下關簽核人員信息
                        ...
                        if (!"2".equals(flowMaintainIsofiles.getRouterStage2())){  //  作廢 結案
                            String initType = "";
                            switch (flowMaintainIsofiles.getRouterStage2()){
                                case "0":
                                   initType = "新增";
                                   break;
                                case "1":
                            ...
                            //將資料移至表中
                            Exter_Insert(formId,initType,flowMaintainIsofiles.getDepartmentName(),flowMaintainIsofiles.getCreateUser(),version);
                            ...
                            /*if("1".equals(flowMaintainIsofiles.getRouterStage2())){
                                ...
                                flowHisDccFiles.setFlag("N");flowHisDccFiles.setUpdateReason(updateReason);
                            ...
                            FlowHisDccFiles insert_record
                                    ...
                                    ,flowMaintainIsofiles.getDepartmentId(),userId,updateReason,new Date(),
                                    "Y","","",l_paper,"","");
                            flowHisDccFilesDao.insertSelective(insert_record);*/
                        }else{
                            /*FlowHisDccFiles flowHisDccFiles = new FlowHisDccFiles(flowMaintainIsofiles.getFileId(),version);
                            flowHisDccFiles.setFlag("N");
                            flowHisDccFiles.setUpdateReason(updateReason);
                        ...
                        email.setUrl(StringUtils.url_ui + "/#/flow/dcc/exterfiles/exterfilesadd?u=4&form_id="+formId);
    ...
    /**
    * @Description: 結案 切狀態不為作廢時插入數據
    * @Param: [formId, initType, departmentName, createUser, version]
    * @return: void
    * @Author: PengFei_Ge
    * @Date: 2020/12/29
    */

    private void Exter_Insert(String formId, String initType, String departmentName, String createUser, String version) {
        List<FlowExternalMessage> flowExternalMessages = flowExternalMessageDao.selectAllByFormId(formId);
        for(int i = 0 ; i<flowExternalMessages.size() ; i++ ){
            if("新增".equals(initType)){
                JsonResult flow_maintain_isofiles = isoService.getNewSysid("flow_maintain_isofiles", flowExternalMessages.get(i).getFileId());
                String file_id = flow_maintain_isofiles.getData().get(0).toString();
                FlowHisDccFiles flowHisDccFiles
                        = new FlowHisDccFiles(file_id,flowExternalMessages.get(i).getFileName(),version,departmentName,
                        createUser,"",new Date(),"Y",flowExternalMessages.get(i).getSupplyDep(),
                        flowExternalMessages.get(i).getSumPage(),"N",flowExternalMessages.get(i).getCustomerId(),
                        flowExternalMessages.get(i).getCustomerVer(),flowExternalMessages.get(i).getSupplyPaper());
                flowHisDccFilesDao.insert(flowHisDccFiles);
                FlowExternalMessage flowExternalMessage = new FlowExternalMessage();
                flowExternalMessage.setFormId(formId);
                flowExternalMessage.setFormLine(flowExternalMessages.get(i).getFormLine());
                flowExternalMessage.setFileNumber(file_id);
                flowExternalMessageDao.updateByformId(flowExternalMessage);
            }
        }
    }
